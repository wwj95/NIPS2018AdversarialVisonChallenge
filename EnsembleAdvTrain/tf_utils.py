...
def tf_train(graph, x, y, data, model_fn, train_mode=None, x_advs=None):
    ...
    if x_advs is not None: # ensemblem adversarial training
        ...
        def aug_1():
            print ("Augmentated")
            return augmentX(padded_input, FLAGS.AUG_RATIO), padded_adv_input, augmentY(y, FLAGS.AUG_RATIO)

        def aug_2():
            return padded_input, padded_adv_input, y
        #if in the test mode, do not need to do augmentation, so apply aug_2()
        if train_mode is None:
            aug_X = padded_input
            X_adv = padded_adv_input
            aug_Y = y
        else:
    ...
    while step < 1:
        ...
        step += 1
    if model.name == 'resnet' :
        model.tf_save(sess, "./models/res_check/resnet18.ckpt")
    else:
