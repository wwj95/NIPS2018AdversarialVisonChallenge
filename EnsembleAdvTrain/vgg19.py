...
class Vgg19:
    ...
    def __init__(self, data_mean, vgg19_npy_path=None, trainable=3, skippable=3, dropout=0.5):
        ...
        if isinstance(trainable, bool):
            self.trainable = [trainable] * 19
        elif isinstance(trainable, int):
            self.trainable = [False] * (19-trainable) + [True] * (trainable)
        else:
            raise ValueError('trainable')

        # skippable
        if isinstance(skippable, bool):
            self.skippable = [skippable] * 19
        elif isinstance(skippable, int):
            self.skippable = [False] * (19-skippable) + [True] * (skippable)
        else:
    ...
    def __call__(self, rgb, train_mode=None):
        ...
        self.pool3 = self.max_pool(self.conv3_4, 'pool3') # [batch_size, (16-2)/2+1, (16-2)/2+1, 256]


        self.conv4_1 = self.conv_layer(self.pool3, 256, 512, "conv4_1", 8) # [batch_size, 8, 8, 512]
        self.conv4_2 = self.conv_layer(self.conv4_1, 512, 512, "conv4_2", 9) # [batch_size, 8, 8, 512]
        self.conv4_3 = self.conv_layer(self.conv4_2, 512, 512, "conv4_3", 10) # [batch_size, 8, 8, 512]
        self.conv4_4 = self.conv_layer(self.conv4_3, 512, 512, "conv4_4", 11) # [batch_size, 8, 8, 512]
        self.pool4 = self.max_pool(self.conv4_4, 'pool4') # [batch_size, 4, 4, 512]

        self.conv5_1 = self.conv_layer(self.pool4, 512, 512, "conv5_1", 12) # [batch_size, 4, 4, 512]
        self.conv5_2 = self.conv_layer(self.conv5_1, 512, 512, "conv5_2", 13) # [batch_size, 4, 4, 512]
        self.conv5_3 = self.conv_layer(self.conv5_2, 512, 512, "conv5_3", 14) # [batch_size, 4, 4, 512]
        self.conv5_4 = self.conv_layer(self.conv5_3, 512, 512, "conv5_4", 15) # [batch_size, 4, 4, 512]
        self.pool5 = self.max_pool(self.conv5_4, 'pool5') # [batch_size, 2, 2, 512]

        self.fc6 = self.fc_layer(self.pool5, 2048, 4096, "fc6", 16)  # 2048 = ((64 // (2 ** 5)) ** 2) * 512
        self.relu6 = tf.nn.relu(self.fc6)
        if self.trainable[16]:
        ...
        self.fc7 = self.fc_layer(self.relu6, 4096, 4096, "fc7", 17)
        self.relu7 = tf.nn.relu(self.fc7)
        if self.trainable[17]:
            if train_mode is not None:
        ...
        self.fc8 = self.fc_layer(self.relu7, 4096, 200, "fc8", 18)
